...
public class SmileLoadingView extends View {
    ...
    public SmileLoadingView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.SmileLoadingStyle);
    }

    public SmileLoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.SmileLoadingView, defStyleAttr, 0);
        mSize = array.getDimensionPixelSize(R.styleable.SmileLoadingView_smile_loading_view_size, SmileDisplayHelper.dp2px(context, 32));
        mPaintColor = array.getInt(R.styleable.SmileLoadingView_android_color, Color.WHITE);
        array.recycle();
        initPaint();
    }

    public SmileLoadingView(Context context, int size, int color) {
        super(context);
        mSize = size;
        mPaintColor = color;
        initPaint();
