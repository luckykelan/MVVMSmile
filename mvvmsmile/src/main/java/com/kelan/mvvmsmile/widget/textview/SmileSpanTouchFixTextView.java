 ...
 * </p>
 * <p>
 * 注意: 使用该 {@link TextView} 时, 用 {@link SmileTouchableSpan} 代替 {@link android.text.style.ClickableSpan},
 * 且同时可以使用 {@link SmileTouchableSpan} 达到修改 span 的文字颜色和背景色的目的。
 * </p>
 * <p>
 * 注意: 使用该 {@link TextView} 时, 需调用 {@link #setMovementMethodDefault()} 方法设置默认的 {@link SmileLinkTouchMovementMethod},
 * TextView 会在 {@link #onTouchEvent(MotionEvent)} 时将事件传递给 {@link SmileLinkTouchMovementMethod},
 * 然后传递给 {@link SmileTouchableSpan}, 实现点击态的变化和点击事件的响应。
 * </p>

 * @see SmileTouchableSpan
 * @see SmileLinkTouchMovementMethod
 */
public class SmileSpanTouchFixTextView extends AppCompatTextView implements ISpanTouchFix {
    /**
    ...
    public SmileSpanTouchFixTextView(Context context) {
        this(context, null);
    }

    public SmileSpanTouchFixTextView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SmileSpanTouchFixTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setMovementMethodDefault(){
        setMovementMethodCompat(SmileLinkTouchMovementMethod.getInstance());
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        mTouchSpanHit = true;
        // 调用super.onTouchEvent,会走到SmileLinkTouchMovementMethod
        // 会走到SmileLinkTouchMovementMethod#onTouchEvent会修改mTouchSpanHint
