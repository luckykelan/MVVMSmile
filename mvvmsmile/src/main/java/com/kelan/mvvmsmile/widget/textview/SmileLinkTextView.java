...
package com.kelan.mvvmsmile.widget.textview;
...
import com.kelan.mvvmsmile.widget.link.SmileLinkTouchMovementMethod;
import com.kelan.mvvmsmile.widget.link.SmileLinkify;
import com.kelan.mvvmsmile.widget.span.SmileOnSpanClickListener;
 ...
 * {@link SmileLinkTextView} 有以下特点:
 * <ul>
 * <li>可以通过 {@link com.kelan.mvvmsmile.R.styleable#SmileLinkTextView} 中的属性设置链接的样式</li>
 * <li>可以通过 {@link SmileLinkTextView#setOnLinkClickListener(SmileLinkTextView.OnLinkClickListener)} 设置链接的点击事件，
 * 而不是 {@link TextView} 默认的 {@link android.content.Intent} 跳转</li>
 * </ul>
 */
public class SmileLinkTextView extends AppCompatTextView implements SmileOnSpanClickListener, ISpanTouchFix {
    private static final String TAG = "LinkTextView";
    private static final int MSG_CHECK_DOUBLE_TAP_TIMEOUT = 1000;
    public static int AUTO_LINK_MASK_REQUIRED = SmileLinkify.PHONE_NUMBERS | SmileLinkify.EMAIL_ADDRESSES | SmileLinkify.WEB_URLS;
    ...
    public SmileLinkTextView(Context context) {
        this(context, null);
        mLinkBgColor = null;
        mLinkTextColor = ContextCompat.getColorStateList(context, R.color.smile_s_link_color);
    }

    public SmileLinkTextView(Context context, ColorStateList linkTextColor, ColorStateList linkBgColor) {
        this(context, null);
        mLinkBgColor = linkBgColor;
        mLinkTextColor = linkTextColor;
    }

    public SmileLinkTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mAutoLinkMaskCompat = getAutoLinkMask() | AUTO_LINK_MASK_REQUIRED;
        setAutoLinkMask(0);
        setMovementMethod(SmileLinkTouchMovementMethod.getInstance());
        setHighlightColor(Color.TRANSPARENT);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.SmileLinkTextView);
        mLinkBgColor = array.getColorStateList(R.styleable.SmileLinkTextView_smile_linkBackgroundColor);
        mLinkTextColor = array.getColorStateList(R.styleable.SmileLinkTextView_smile_linkTextColor);
        array.recycle();
        if (mOriginText != null) {
            setText(mOriginText);
        }
    ...
    public void setText(CharSequence text, BufferType type) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            SmileLinkify.addLinks(builder, mAutoLinkMaskCompat, mLinkTextColor, mLinkBgColor, this);
            text = builder;
        }
