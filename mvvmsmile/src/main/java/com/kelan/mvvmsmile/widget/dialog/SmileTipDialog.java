...
import com.kelan.mvvmsmile.widget.SmileLoadingView;
...
/**
 * 提供一个浮层展示在屏幕中间, 一般使用 {@link SmileTipDialog.Builder} 或 {@link SmileTipDialog.CustomBuilder} 生成。
 * <ul>
 * <li>{@link SmileTipDialog.Builder} 提供了一个图标和一行文字的样式, 其中图标有几种类型可选, 见 {@link SmileTipDialog.Builder.IconType}</li>
 * <li>{@link SmileTipDialog.CustomBuilder} 支持传入自定义的 layoutResId, 达到自定义 TipDialog 的效果。</li>
 * </ul>
 */

public class SmileTipDialog extends Dialog {

    public SmileTipDialog(Context context) {
        this(context, R.style.SmileUI_TipDialog);
    }

    public SmileTipDialog(Context context, int themeResId) {
    ...
    public static class Builder {
         ...
         * @return 创建的 Dialog
         */
        public SmileTipDialog create(boolean cancelable) {
            SmileTipDialog dialog = new SmileTipDialog(mContext);
            dialog.setCancelable(cancelable);
            dialog.setContentView(R.layout.smile_tip_dialog_layout);
            ViewGroup contentWrap = (ViewGroup) dialog.findViewById(R.id.contentWrap);

            if (mCurrentIconType == ICON_TYPE_LOADING) {
                SmileLoadingView loadingView = new SmileLoadingView(mContext);
                loadingView.setColor(Color.WHITE);
                loadingView.setSize(SmileDisplayHelper.dp2px(mContext, 32));
                ...
                if (mCurrentIconType == ICON_TYPE_SUCCESS) {
                    imageView.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.smile_icon_notify_done));
                } else if (mCurrentIconType == ICON_TYPE_FAIL) {
                    imageView.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.smile_icon_notify_error));
                } else {
                    imageView.setImageDrawable(ContextCompat.getDrawable(mContext, R.drawable.smile_icon_notify_info));
                }
            ...
            if (mTipWord != null && mTipWord.length() > 0) {
                ...
                if (mCurrentIconType != ICON_TYPE_NOTHING) {
                    tipViewLP.topMargin = SmileDisplayHelper.dp2px(mContext, 12);
                }
                ...
                tipView.setTextColor(ContextCompat.getColor(mContext, R.color.smile_config_color_white));
