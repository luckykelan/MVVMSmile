...
public class SmileResHelper {
    ...
    public static Drawable getAttrDrawable(Context context, TypedArray typedArray, int index){
        ...
        if(value != null){
            if(value.type != TypedValue.TYPE_ATTRIBUTE && value.resourceId != 0){
                return SmileDrawableHelper.getVectorDrawable(context, value.resourceId);
            }
        }
        return null;
    }
    ...
    public static int getAttrDimen(Context context, int attrRes){
        TypedValue typedValue = new TypedValue();
        context.getTheme().resolveAttribute(attrRes, typedValue, true);
        return TypedValue.complexToDimensionPixelSize(typedValue.data, SmileDisplayHelper.getDisplayMetrics(context));
    }

    public static void assignTextViewWithAttr(TextView textView, int attrRes){
        TypedArray a = textView.getContext().obtainStyledAttributes(null, R.styleable.SmileTextCommonStyleDef, attrRes, 0);
        ...
        for(int i = 0; i < count; i++){
            ...
            if(attr == R.styleable.SmileTextCommonStyleDef_android_gravity){
                textView.setGravity(a.getInt(attr, -1));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_textColor){
                textView.setTextColor(a.getColorStateList(attr));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_textSize){
                textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, a.getDimensionPixelSize(attr, 0));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_paddingLeft){
                paddingLeft = a.getDimensionPixelSize(attr, 0);
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_paddingRight){
                paddingRight = a.getDimensionPixelSize(attr, 0);
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_paddingTop){
                paddingTop = a.getDimensionPixelSize(attr, 0);
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_paddingBottom){
                paddingBottom = a.getDimensionPixelSize(attr, 0);
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_singleLine){
                textView.setSingleLine(a.getBoolean(attr, false));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_ellipsize){
            ...
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_maxLines){
                textView.setMaxLines(a.getInt(attr, -1));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_background){
                SmileViewHelper.setBackgroundKeepingPadding(textView, a.getDrawable(attr));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_lineSpacingExtra){
                textView.setLineSpacing(a.getDimensionPixelSize(attr, 0), 1f);
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_drawablePadding){
                textView.setCompoundDrawablePadding(a.getDimensionPixelSize(attr, 0));
            }else if(attr == R.styleable.SmileTextCommonStyleDef_android_textColorHint){
