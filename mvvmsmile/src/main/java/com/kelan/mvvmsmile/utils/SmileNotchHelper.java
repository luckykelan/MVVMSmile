...
public class SmileNotchHelper {

    private static final String TAG = "SmileNotchHelper";
    ...
    public static boolean has3rdNotch(Context context){
        if (SmileDeviceHelper.isHuawei()) {
            return hasNotchInHuawei(context);
        } else if (SmileDeviceHelper.isVivo()) {
            return hasNotchInVivo(context);
        } else if (SmileDeviceHelper.isOppo()) {
            return hasNotchInOppo(context);
        } else if (SmileDeviceHelper.isXiaomi()) {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (SmileDeviceHelper.isHuawei()) {
            boolean isHuaweiNotchSetToShow = SmileDisplayHelper.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        ...
        if (SmileDeviceHelper.isVivo()) {
            ...
            rect.top = SmileStatusBarHelper.getStatusbarHeight(context);
            rect.bottom = 0;
        } else if (SmileDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (SmileDeviceHelper.isVivo()) {
            ...
            rect.left = SmileStatusBarHelper.getStatusbarHeight(context);
            rect.right = 0;
        } else if (SmileDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (SmileDeviceHelper.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = SmileStatusBarHelper.getStatusbarHeight(context);
        } else if (SmileDeviceHelper.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (SmileDeviceHelper.isVivo()) {
            ...
            rect.right = SmileStatusBarHelper.getStatusbarHeight(context);
            rect.left = 0;
        } else if (SmileDeviceHelper.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return SmileDisplayHelper.getStatusBarHeight(context);
    }

    public static int getNotchWidthInVivo(Context context){
        return SmileDisplayHelper.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return SmileDisplayHelper.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
       return SmileDeviceHelper.isXiaomi() && SmileNotchHelper.hasNotch(view);
    }
