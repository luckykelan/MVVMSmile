...
import com.kelan.mvvmsmile.widget.SmileInterpolatorStaticHolder;
 ...
 * <ul>
 * <li>可以用 xml 和 SmileTabSegment 提供的 set 方法统一配置文字颜色、icon 位置、是否要下划线等</li>
 * <li>每个 Tab 都可以非常灵活的配置，如果没有提供相关配置，则使用 SmileTabSegment 提供的配置，具体参考 {@link SmileTab}</li>
 ...
 * SmileTabSegment 会将 {@link PagerAdapter#getPageTitle(int)} 返回的字符串作为 Tab 的文案
 * </li>
 * <li>
 * 如果你希望自己设置 Tab 的文案或图片，那么通过{@link #addTab(SmileTab)}添加 Tab:
 * <code>
 * SmileTabSegment mTabSegment = new SmileTabSegment((getContext());
 * // config mTabSegment
 * SmileTabBuilder tabBuilder = mTabSegment.tabBuilder()
 ...
 * </li>
 * <li>
 * 如果你想更改tab,则调用{@link #updateTabText(int, String)} 或者 {@link #replaceTab(int, SmileTab)}
 * <code>
 ...
 * @date 2016-01-27
 */
public class SmileTabSegment extends HorizontalScrollView {

    private static final String TAG = "SmileTabSegment";
    ...
    private SmileTabIndicator mIndicator = null;
    ...
    private SmileTabAdapter mTabAdapter;

    private SmileTabBuilder mTabBuilder;
    ...
    public SmileTabSegment(Context context) {
        this(context, null);
    }

    public SmileTabSegment(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.SmileTabSegmentStyle);
    }

    public SmileTabSegment(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray array = context.obtainStyledAttributes(attrs,
                R.styleable.SmileTabSegment, defStyleAttr, 0);

        // indicator
        boolean hasIndicator = array.getBoolean(R.styleable.SmileTabSegment_smile_tab_has_indicator, false);
        int indicatorHeight = array.getDimensionPixelSize(
                R.styleable.SmileTabSegment_smile_tab_indicator_height,
                getResources().getDimensionPixelSize(R.dimen.smile_tab_segment_indicator_height));
        boolean indicatorTop = array.getBoolean(R.styleable.SmileTabSegment_smile_tab_indicator_top, false);
        boolean indicatorWidthFollowContent = array.getBoolean(
                R.styleable.SmileTabSegment_smile_tab_indicator_with_follow_content, false);
        mIndicator = createTabIndicatorFromXmlInfo(hasIndicator, indicatorHeight,
                indicatorTop, indicatorWidthFollowContent);

        // tabBuilder
        int normalTextColor = array.getColor(R.styleable.SmileTabSegment_smile_tab_normal_color,
                ContextCompat.getColor(context, R.color.smile_config_color_gray_5));
        int selectedTextColor = array.getColor(R.styleable.SmileTabSegment_smile_tab_selected_color,
                SmileResHelper.getAttrColor(context, R.attr.smile_config_color_blue));
        int normalTextSize = array.getDimensionPixelSize(
                R.styleable.SmileTabSegment_android_textSize,
                getResources().getDimensionPixelSize(R.dimen.smile_tab_segment_text_size));
        normalTextSize = array.getDimensionPixelSize(
                R.styleable.SmileTabSegment_smile_tab_normal_text_size, normalTextSize);
        int selectedTextSize = normalTextSize;
        selectedTextSize = array.getDimensionPixelSize(
                R.styleable.SmileTabSegment_smile_tab_selected_text_size, selectedTextSize);
        mTabBuilder = new SmileTabBuilder(context)
                .setColor(normalTextColor, selectedTextColor)
                .setTextSize(normalTextSize, selectedTextSize)
                .setIconPosition(array.getInt(R.styleable.SmileTabSegment_smile_tab_icon_position,
                        SmileTab.ICON_POSITION_LEFT));
        mMode = array.getInt(R.styleable.SmileTabSegment_smile_tab_mode, MODE_FIXED);
        mItemSpaceInScrollMode = array.getDimensionPixelSize(
                R.styleable.SmileTabSegment_smile_tab_space, SmileDisplayHelper.dp2px(context, 10));
        array.recycle();
    ...
    protected SmileTabAdapter createTabAdapter(ViewGroup tabParentView) {
        return new SmileTabAdapter(this, tabParentView);
    }

    protected SmileTabIndicator createTabIndicatorFromXmlInfo(boolean hasIndicator,
                                                             int indicatorHeight,
        ...
        return new SmileTabIndicator(indicatorHeight, indicatorTop, indicatorWidthFollowContent);
    }

    public SmileTabBuilder tabBuilder() {
        // do not change mTabBuilder to keep common config not changed
        return new SmileTabBuilder(mTabBuilder);
    }
     ...
     */
    public void setIndicator(@Nullable SmileTabIndicator indicator) {
    ...
    /**
     * add a tab to SmileTabSegment
     *
     * @param tab SmileTab
     * @return return this to chain
     */
    public SmileTabSegment addTab(SmileTab tab) {
    ...
    /**
     * notify dataChanged event to SmileTabSegment
     */
    ...
    void onClickTab(int index) {
        ...
        SmileTab model = mTabAdapter.getItem(index);
        if (model != null) {
    ...
    /**
     * associate SmileTabSegment with a {@link ViewPager}
     *
     * @param viewPager       the ViewPager to associate
     * @param useAdapterTitle populate the tab with viewPager.adapter.getTitle
     * @param autoRefresh     refresh SmileTabSegment when viewPager.adapter changed.
     */
    ...
    public void selectTab(final int index, boolean noAnimation, boolean fromTabClick) {
        ...
        List<SmileTabView> listViews = mTabAdapter.getViews();
        ...
        final SmileTab prevModel = mTabAdapter.getItem(prev);
        final SmileTabView prevView = listViews.get(prev);
        final SmileTab nowModel = mTabAdapter.getItem(index);
        final SmileTabView nowView = listViews.get(index);
        ...
        animator.setInterpolator(SmileInterpolatorStaticHolder.LINEAR_INTERPOLATOR);
    ...
    private void layoutIndicator(SmileTab model, boolean invalidate) {
    ...
    private void layoutIndicatorInTransition(SmileTab preModel, SmileTab targetModel, float offsetPercent) {
        if (mIndicator == null) {
            return;
        }
        ...
        int indicatorColor = SmileColorHelper.computeColor(
    ...
    public void updateIndicatorPosition(final int index, float offsetPercent) {
        ...
        final List<SmileTabView> listViews = mTabAdapter.getViews();
        if (listViews.size() <= index || listViews.size() <= targetIndex) {
            return;
        }
        SmileTab preModel = mTabAdapter.getItem(index);
        SmileTab targetModel = mTabAdapter.getItem(targetIndex);
        SmileTabView preView = listViews.get(index);
        SmileTabView targetView = listViews.get(targetIndex);
     ...
     * @param model 新的 Tab
     */
    public void replaceTab(int index, SmileTab model) {
        try {
    ...
    /**
     * get {@link SmileTab} by index
     *
     * @param index index
     * @return SmileTab
     */
    public SmileTab getTab(int index) {
    ...
    public void showSignCountView(Context context, int index, int count) {
        SmileTab tab = mTabAdapter.getItem(index);
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<SmileTabSegment> mTabSegmentRef;

        public TabLayoutOnPageChangeListener(SmileTabSegment tabSegment) {
            mTabSegmentRef = new WeakReference<>(tabSegment);
        }

        @Override
        public void onPageScrollStateChanged(final int state) {
            final SmileTabSegment tabSegment = mTabSegmentRef.get();
            if (tabSegment != null) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mCurrentSelectedIndex != NO_POSITION && mMode == MODE_SCROLLABLE) {
            final SmileTabView view = mTabAdapter.getViews().get(mCurrentSelectedIndex);
    ...
    private final class Container extends ViewGroup {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            List<SmileTabView> childViews = mTabAdapter.getViews();
            ...
            if (mMode == MODE_FIXED) {
                ...
                for (i = 0; i < size; i++) {
                    ...
                    // reset
                    SmileTab tab = mTabAdapter.getItem(i);
