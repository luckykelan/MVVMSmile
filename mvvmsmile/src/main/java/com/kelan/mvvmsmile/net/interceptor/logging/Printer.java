package com.kelan.mvvmsmile.net.interceptor.logging;
...
class Printer {
    ...
    static void printJsonRequest(LoggingInterceptor.Builder builder, Request request) {
        String requestBody = LINE_SEPARATOR + BODY_TAG + LINE_SEPARATOR + bodyToString(request,builder.getRsaHelper()) ;
    ...
    private static String bodyToString(final Request request, RSAHelper rsaHelper) {
        try {
    ...
    static String getJsonString(String msg, RSAHelper rsaHelper) {
        //msg = rsaHelper.decryptWithBase64(msg);
        String message;
        try {
            if (msg.startsWith("{")) {
