...
import com.kelan.mvvmsmile.widget.layout.SmileLinearLayout;
...
/**
 * 创建 {@link SmileDialog} 的 Builder 基类, 不同的 Builder 子类拥有创建不同类型对话框的能力, 具体见子类。
 * <p>该类产生的 Dialog 分为上中下三个部分:</p>
 * <ul>
 * <li>上部分是 title 区域, 支持显示纯文本标题, 通过 {@link #setTitle(int)} 系列方法设置。
 * 子类也可以通过 override {@link #onCreateTitle(SmileDialog, ViewGroup, Context)} 方法自定义</li>
 * <li>中间部分的内容由各个子类决定, 子类通过 override {@link #onCreateContent(SmileDialog, ViewGroup, Context)} 方法自定义。</li>
 * <li>下部分是操作区域, 支持添加操作按钮, 通过 {@link #addAction(int, int, SmileDialogAction.ActionListener)} 系列方法添加。
 * 子类也可以通过 override {@link #onCreateHandlerBar(SmileDialog, ViewGroup, Context)} 方法自定义。
 * 其中操作按钮有内联和块级之分, 也有普通、正向、反向之分, 具体见 {@link SmileDialogAction}
 * </li>
 * </ul>
 *
 * @author cginechen
 * @date 2015-10-20
 */
public abstract class SmileDialogBuilder<T extends SmileDialogBuilder> {
    ...
    public static void setOnProvideDefaultTheme(OnProvideDefaultTheme onProvideDefaultTheme) {
        SmileDialogBuilder.sOnProvideDefaultTheme = onProvideDefaultTheme;
    }

    private Context mContext;
    protected SmileDialog mDialog;
    protected String mTitle;
    private boolean mCancelable = true;
    private boolean mCanceledOnTouchOutside = true;

    protected LinearLayout mRootView;
    protected SmileDialogView mDialogView;
    protected View mAnchorTopView;
    protected View mAnchorBottomView;
    protected List<SmileDialogAction> mActions = new ArrayList<>();
    private SmileDialogView.OnDecorationListener mOnDecorationListener;

    protected TextView mTitleView;
    protected SmileLinearLayout mActionContainer;
    ...
    private int mActionDividerColorRes = R.color.smile_config_color_separator;
    private int mActionDividerInsetStart = 0;
    private int mActionDividerInsetEnd = 0;

    public SmileDialogBuilder(Context context) {
    ...
    protected int getContentAreaMaxHeight() {
        if (mContentAreaMaxHeight == -1) {
            ...
            return (int) (SmileDisplayHelper.getScreenHeight(mContext) * 0.85) - SmileDisplayHelper.dp2px(mContext, 100);
        }
    ...
    public T setTitle(String title) {
        if (title != null && title.length() > 0) {
            this.mTitle = title + mContext.getString(R.string.smile_tool_fixellipsize);
        }
        return (T) this;
    }
    ...
    @SuppressWarnings("unchecked")
    public T setOnDecorationListener(SmileDialogView.OnDecorationListener onDecorationListener) {
     ...
     */
    @SuppressWarnings("unchecked")
    public T addAction(@Nullable SmileDialogAction action) {
        if (action != null) {
            mActions.add(action);
        }
     ...
     */
    public T addAction(int strResId, SmileDialogAction.ActionListener listener) {
     ...
     * @param listener 点击回调事件
     */
    public T addAction(CharSequence str, SmileDialogAction.ActionListener listener) {
        return addAction(0, str, SmileDialogAction.ACTION_PROP_NEUTRAL, listener);
    }
     ...
     * @param listener  点击回调事件
     */
    public T addAction(int iconResId, int strResId, SmileDialogAction.ActionListener listener) {
        return addAction(iconResId, strResId, SmileDialogAction.ACTION_PROP_NEUTRAL, listener);
    }
     ...
     * @param listener  点击回调事件
     */
    public T addAction(int iconResId, CharSequence str, SmileDialogAction.ActionListener listener) {
        return addAction(iconResId, str, SmileDialogAction.ACTION_PROP_NEUTRAL, listener);
    }
     ...
     * @param listener 点击回调事件
     */
    public T addAction(int iconRes, int strRes, @SmileDialogAction.Prop int prop, SmileDialogAction.ActionListener listener) {
     ...
     */
    @SuppressWarnings("unchecked")
    public T addAction(int iconRes, CharSequence str, @SmileDialogAction.Prop int prop, SmileDialogAction.ActionListener listener) {
        SmileDialogAction action = new SmileDialogAction(mContext, iconRes, str, prop, listener);
        mActions.add(action);
        return (T) this;
    }
     ...
     * 产生一个 Dialog 并显示出来
     */
    public SmileDialog show() {
        final SmileDialog dialog = create();
        dialog.show();
        return dialog;
    }
    ...
    public SmileDialog create() {
        ...
        return create(R.style.SmileUI_Dialog);
    }
    ...
    public SmileDialog create(@StyleRes int style) {
        mDialog = new SmileDialog(mContext, style);
        Context dialogContext = mDialog.getContext();

        mRootView = (LinearLayout) LayoutInflater.from(dialogContext).inflate(
                R.layout.smile_dialog_layout, null);
        mDialogView = (SmileDialogView) mRootView.findViewById(R.id.dialog);
     ...
     * 创建顶部的标题区域
     */
    protected void onCreateTitle(SmileDialog dialog, ViewGroup parent, Context context) {
        if (hasTitle()) {
            mTitleView = new TextView(context);

            mTitleView.setText(mTitle);
            SmileResHelper.assignTextViewWithAttr(mTitleView, R.attr.smile_dialog_title_style);
     ...
     * 创建中间的区域
     */
    protected abstract void onCreateContent(SmileDialog dialog, ViewGroup parent, Context context);

    /**
     * 创建底部的操作栏区域
     */
    protected void onCreateHandlerBar(final SmileDialog dialog, ViewGroup parent, Context context) {
        int size = mActions.size();
        if (size > 0) {
            TypedArray a = context.obtainStyledAttributes(null, R.styleable.SmileDialogActionContainerCustomDef, R.attr.smile_dialog_action_container_style, 0);
            int count = a.getIndexCount();
            int justifyContent = 1, spaceCustomIndex = 0;
            int actionHeight = -1, actionSpace = 0;
            for (int i = 0; i < count; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.SmileDialogActionContainerCustomDef_smile_dialog_action_container_justify_content) {
                    justifyContent = a.getInteger(attr, justifyContent);
                } else if (attr == R.styleable.SmileDialogActionContainerCustomDef_smile_dialog_action_container_custom_space_index) {
                    spaceCustomIndex = a.getInteger(attr, 0);
                } else if (attr == R.styleable.SmileDialogActionContainerCustomDef_smile_dialog_action_space) {
                    actionSpace = a.getDimensionPixelSize(attr, 0);
                } else if (attr == R.styleable.SmileDialogActionContainerCustomDef_smile_dialog_action_height) {
            ...
            mActionContainer = new SmileLinearLayout(context, null, R.attr.smile_dialog_action_container_style);
            ...
            for (int i = 0; i < size; i++) {
                ...
                SmileDialogAction action = mActions.get(i);
                ...
                SmileButton actionView = action.buildActionView(mDialog, i);
                    ...
                    public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
                        ...
                        if (childCount > 0) {
                            ...
                            if (lastChild.getRight() > width) {
                                int childPaddingHor = Math.max(0, lastChild.getPaddingLeft() - SmileDisplayHelper.dp2px(mContext, 3));
    ...
    protected void onAfter(SmileDialog dialog, LinearLayout parent, Context context) {
    ...
    public List<SmileDialogAction> getPositiveAction() {
        List<SmileDialogAction> output = new ArrayList<>();
        for (SmileDialogAction action : mActions) {
            if (action.getActionProp() == SmileDialogAction.ACTION_PROP_POSITIVE) {
                output.add(action);
            }
        }
        return output;
    ...
    public interface OnProvideDefaultTheme {
        int getThemeForBuilder(SmileDialogBuilder builder);
    }
}
