...
public class SmileTabAdapter extends SmileItemViewsAdapter<SmileTab, SmileTabView> implements SmileTabView.Callback {
    ...
    public SmileTabAdapter(SmileTabSegment tabSegment, ViewGroup parentView) {
        super(parentView);
        mTabSegment = tabSegment;
    }

    @Override
    protected SmileTabView createView(ViewGroup parentView) {
        return new SmileTabView(parentView.getContext());
    }

    @Override
    protected final void bind(SmileTab item, SmileTabView view, int position) {
        onBindTab(item, view, position);
        view.setCallback(this);
    }

    protected void onBindTab(SmileTab item, SmileTabView view, int position) {
        view.bind(item);
    }

    @Override
    public void onClick(SmileTabView view) {
        int index = getViews().indexOf(view);
        mTabSegment.onClickTab(index);
    }

    @Override
    public void onDoubleClick(SmileTabView view) {
        int index = getViews().indexOf(view);
        mTabSegment.onDoubleClick(index);
    }

    @Override
    public void onLongClick(SmileTabView view) {
    }
}
