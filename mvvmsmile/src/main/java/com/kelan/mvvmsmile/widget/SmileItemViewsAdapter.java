...
import com.kelan.mvvmsmile.widget.tab.SmileTab;
 ...
 * 复用已存在的 {@link View}。
 * SmileUI 用于 {@link SmileTabSegment 中 {@link SmileTab } 与数据的适配。
 *
 * @author cginechen
 * @date 2016-11-27
 */

public abstract class SmileItemViewsAdapter<T, V extends View> {
    ...
    public SmileItemViewsAdapter(ViewGroup parentView) {
    ...
    public void detach(int count) {
        ...
        while (childCount > 0 && count > 0) {
            ...
            Object notCacheTag = view.getTag(R.id.smile_view_can_not_cache_tag);
    ...
    public SmileItemViewsAdapter<T, V> addItem(T item) {
        mItemData.add(item);
        return this;
    }
